...
/**
 * Created by hanshuaiqi 18:57 2017/11/6
 */
@Service("ucreditNewService")
public class UcreditNewServiceImpl implements LoanStrategyService {
    public static final Logger logger = LoggerFactory.getLogger(UcreditNewServiceImpl.class);
    @Resource
    ...
    public LoanStrategyResult execute(String name, String phone, String idNo, UserScoreStatistics userScore, String mobileType, String black_box) {
        ...
        if (header.getString("ret_code").equals("000000")) {
            Integer ret_score = json.getJSONObject("body").getJSONObject("scores").getInteger("MODEL2");
            if(ret_score!=null){
                result.setScore(Double.valueOf(ret_score));
                userScore.setNewUdScore(BigDecimal.valueOf(ret_score));
                result.setStatus("20");
            }else{
                result.setStatus("10");
                result.setScore(-1D);
                userScore.setNewUdScore(BigDecimal.valueOf(-1));
            }
        } else {
            result.setStatus("10");
            result.setScore(-1D);
